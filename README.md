# CS3807---Deep-Learning-Lab

This repository contains the experiments completed as part of the **CS3807 – Deep Learning Laboratory** course.

## Experiments

| Experiment | Date | Title | Brief Description |
|---|---|---|---|
| **Experiment 1** | 10/07/2026 | **Implementation of a Single Layer Perceptron for Binary Classification** | Implemented a Single Layer Perceptron from scratch and studied the perceptron learning algorithm, activation functions, learning-rate effects, decision boundaries, and binary classification performance using the UCI Banknote Authentication dataset. |
| **Experiment 2** | 17/07/2026 | **Implementation of a Multi-Layer Perceptron (MLP) for Multi-Class Image Classification** | Built and evaluated a Multi-Layer Perceptron using TensorFlow/Keras for multi-class image classification on Fashion-MNIST. The experiment included preprocessing, model training, performance evaluation, hyperparameter optimization, and an additional XOR analysis. |
| **Experiment 3** | 31/07/2026 | **Implementation of Convolutional Neural Networks (CNNs) for Image Classification** | Implemented and evaluated a CNN for CIFAR-10 image classification while studying convolution, kernel sizes, stride, padding, pooling operations, feature maps, learned filters, and CNN performance metrics. |
| **Experiment 4** | 20/08/2026 | **Comparative Study of Deep Convolutional Neural Network Architectures Using Transfer Learning** | Studied and compared LeNet-5, AlexNet, GoogLeNet, VGG16, ResNet50, and MobileNetV2 using the CIFAR-10 dataset. The experiment included CNN architecture analysis, transfer learning, fine-tuning, hyperparameter studies, evaluation using precision, recall and F1-score, confusion-matrix analysis, and comparison of models based on accuracy, parameter count, and training time. |
| **Experiment 5** | 27/08/2026 | **Performance Optimization and Regularization Techniques for Deep Neural Networks** | Investigated the effects of weight initialization, batch normalization, regularization techniques, optimizers, learning rate, batch size, and dropout on deep neural network training and generalization. The experiment also included transfer learning and fine-tuning, 5-fold cross-validation, and final model evaluation using accuracy, precision, recall, F1-score, confusion-matrix analysis, and misclassified-image analysis. |
